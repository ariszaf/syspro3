...
FifoFD HTTPdeamon::fifoFD;
bool HTTPdeamon::online = true;
Statistics HTTPdeamon::statistics;
char* HTTPdeamon::dirName;

HTTPdeamon::HTTPdeamon(char * rootdirName, int t, int servingport, int commandport) : servingport(servingport), commandport(commandport) {
...
int HTTPdeamon::openListeningSocket(int port) {
    int sock;
...
void HTTPdeamon::initialize() {
    // create sockets
    servingfd = openListeningSocket(servingport);
...
void HTTPdeamon::createThreads() { //dimiourgia twn worker 
    for (int i = 0; i < numOfThreads; i++) {
        pthread_t p;
...
void HTTPdeamon::enqueue(int fd) {
    struct sockaddr_in client;
    socklen_t clientlen = sizeof (client);
    struct sockaddr *clientptr = (struct sockaddr *) &client;

    cout << "Httpd: connection from web" << endl;
    int newsock;
    if ((newsock = accept(fd, clientptr, &clientlen)) < 0) {
        perror_exit("accept");
    }

    fifoFD.place(newsock);
}

void HTTPdeamon::submit(int fd) {
    struct sockaddr_in client;
    socklen_t clientlen = sizeof (client);
    struct sockaddr *clientptr = (struct sockaddr *) &client;
    NetworkFunctions messenger;

    cout << "Httpd: connection from console" << endl;
...
void HTTPdeamon::restart(pid_t terminated_pid) {
...
void HTTPdeamon::perror_exit(const char *message) {
    perror(message);
    exit(EXIT_FAILURE);
}

int HTTPdeamon::getCommandfd() {
    return commandfd;
}

void HTTPdeamon::setCommandfd(int commandfd) {
    this->commandfd = commandfd;
}

int HTTPdeamon::getServingfd() {
    return servingfd;
}

void HTTPdeamon::setServingfd(int servingfd) {
    this->servingfd = servingfd;
}

...
void Worker::start() {
    ...
    while (1) {
        // obtain from fifo
        int sock = HTTPdeamon::fifoFD.obtain();

        cout << "worker " << pthread_self() << " consumed " << sock << endl;
        ...
        // read GET request
        char * header = messenger.readHTTPheader(sock);
                            ...
                            if (position == 2) {
                                if (strlen(token) > 3) {
                                   ...
                                   cout << "error to deytero token einai mikrotero apo 3" << endl;
                                    ...
                                    messenger.sendTcpMessage(sock, response);
                                    bdcheck = true;
                                    break;
                                }
                            } else if (position == 3) {
        ...
        if (bdcheck == false) {
            char* fullURL = new char [strlen(URL) + strlen(HTTPdeamon::dirName) + 1]();
            // strcat(fullURL, "/");
            strcat(fullURL, HTTPdeamon::dirName);
            strcat(fullURL, URL);
